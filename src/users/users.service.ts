...
export class UsersService {
    ...
    async findUserById(id: string) {
        return  await this.userModel.findById(id).populate({path:"friends",populate:{
    ...
    async acceptFriend(adderId: string, recieverId: string) {
        ...
        try {
            ...
            const friendDocs = await this.friendModel.find({ '_id': { $in: reciever.friends } }).populate({path:'requester',select:'-password'}).populate({path:'recipient',select:'-password'}).exec()
            return friendDocs
