...
export class ConversationService {
    ...
    async initConversation(starterId: string, partnerId: string) {
        const starter = await this.usersService.findUserById(starterId) // user 1
        const partner = await this.usersService.findUserById(partnerId) // user 2
        ...
        const newConversation = new this.conversationModel() // add two members to the conversation
        ...
        try {
            ...
            return await this.conversationModel.find({ // query the conversations of user along with messages
                '_id': {
